...
# Para el acknowledge manda un único frame de 8 bits, que contiene el número
# de secuencia del frame de datos al que se le hace acknowledge

STOPANDWAIT_DATA_HEADER_SIZE = 4
STOPANDWAIT_DATA_FRAME_SIZE = STOPANDWAIT_DATA_HEADER_SIZE + CHUNK_SIZE
STOPANDWAIT_ACK_FRAME_SIZE = 1

def encode_stopandwait_data_frame(is_last, seq, data):
    return struct.pack('!bbh', bool(is_last), seq, len(data)) + pad_to_chunk_size(data)

def decode_stopandwait_data_frame(frame):
    (is_last, seq, length) = struct.unpack('!bbh', frame[:STOPANDWAIT_DATA_HEADER_SIZE])
    assert is_last in (0, 1)
    padded_data = frame[STOPANDWAIT_DATA_HEADER_SIZE:]
    data = padded_data[:length]
    return bool(is_last), seq, data

def encode_stopandwait_ack_frame(seq):
    return struct.pack('!b', seq)

def decode_stopandwait_ack_frame(frame):
    (seq,) = struct.unpack('!b', frame[:STOPANDWAIT_ACK_FRAME_SIZE])
    return seq
